...
Friend Class frmMain
	...
	Delegate Sub RunTest(ByVal Barcode As String, ByVal TestPhase As list(Of String), ByRef Cancel As Boolean)
	...
	Private Sub CmdRunAll_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles CmdRunAll.Click
		...
		Dim tmpTestStep As String
		Dim TestStepL As New List(Of String)
		...
		With Me.LstTestStep
			For Each row As DataGridViewRow In .Rows
				If row.Cells(0).Value = True Then
					tmpTestStep = row.Cells(1).Value.ToString
					tmpTestStep = Replace(tmpTestStep, " ", "_")
					TestStepL.Add(tmpTestStep)
				End If
		...
		For Each tmpTestStep In TestStepL
			SetStepStatus(tmpTestStep, 1)
		Next


		If TestObj Is Nothing Then
			Common.clsOperator.REvntPreTest(TestStepL, tmpBarcode, Cancel)
		Else
			Call TestObj.PreTest(TestStepL, tmpBarcode, Cancel)
		End If
		...
		Call pResults.StartTest(tmpBarcode, TestStepL(0), (Common.CelesticaBarcode))

		If TestObj Is Nothing Then
			Common.clsOperator.REvntRunTest(Me.TxtBarcode.Text, TestStepL, Cancel)
		Else
			'Call TestObj.RunTest(Me.TxtBarcode.Text, Replace(TestStep, "Invalid_", ""), Cancel)
			Dim dgRun As New RunTest(AddressOf TestObj.RunTest)
			dgRun.BeginInvoke(Me.TxtBarcode.Text, TestStepL, Cancel, AddressOf EndTest, TestStepL)
			'Dim res As IAsyncResult = dgRun.BeginInvoke(Me.TxtBarcode.Text, Replace(TestStep, "Invalid_", ""), Cancel, Nothing, Nothing)
			'While res.IsCompleted = False
			'  Threading.Thread.Sleep(50)
			'  Application.DoEvents()
			'End While
			'dgRun.EndInvoke(Cancel, res)
		End If
	...
	Private Sub EndTest()
		...
		End If
	End Sub
	Private Sub EndTest(ByVal ar As IAsyncResult)
		Dim frmTestComplete As New frmTestComplete
		Call clsOperator.StopTestGroup()
		Call pResults.StopTest()
		...
		Dim TestStepL As List(Of String) = CType(ar.AsyncState, List(Of String))
		Dim TestStep As String

		For Each TestStep In TestStepL
			SetStepStatus(TestStep, 4)
		Next
		...
		If (pResults.GetFailures() + pResults.GetPasses) > 0 Then
			...
			Dim TestStat As String = pResults.GetTestStatus
			If Common.PassingUnits And TestStat = "PASS" Then Call pResults.PrintData_2(, False)
			If Common.FailingUnits And TestStat = "FAIL" Then Call pResults.PrintData_2(, False)
			If Common.FailingGroups And TestStat = "FAIL" Then Call pResults.PrintData_2(, True)
	...
	Private Sub LstTestStep_CellClick(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles LstTestStep.CellClick
		ShowSelectedStep()
	End Sub
	Private Sub ShowSelectedStep()
		With LstTestStep
			...
			Call UpdateGroupsGrid(LstTestStep.CurrentRow.Cells(1).Value.ToString)
		End With
	End Sub
	...
	Public Sub mnuCalibrateCustom_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs)
		If (TypeOf eventSender Is ToolStripMenuItem) Then
			...
			Try
				...
				pCheck.TestStepL.Clear()
				pCheck.TestStepL.Add(CalName)
	...
	Private Sub DataGridView2_CellContentDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataGridView2.CellContentDoubleClick, LstTestStep.CellClick
		'Dim tmpProcess As String
		'Dim TGName As String

		'With Me.LstTestStep
		'	tmpProcess = Me.LstTestStep.CurrentRow.Cells(1).Value.ToString
		'End With
	...
	Private Sub EnableForm(ByVal Enable As Boolean)
		If InvokeRequired Then
			Me.BeginInvoke(New Action(Of Boolean)(AddressOf EnableForm), Enable)
		Else
	...
	Public Sub ClearResults()
		...
		For i = 0 To Me.LstTestStep.Rows.Count - 1
			Call SetStepStatus(Me.LstTestStep.Rows(i).Cells(1).Value, 0)
		Next
	...
	Public Sub SetGroupStatus(ByVal TestGroup As String, ByVal Status As Short)
		...
		Select Case Status
			...
			Case 4
				Me.DataGridView2.Rows(i).Cells(1).Value = "Abort"
				Me.DataGridView2.Rows(i).Cells(1).Style = redCell
		End Select
	...
	Public Sub SetStepStatus(ByVal TestStep As String, ByVal Status As Short)
		If InvokeRequired Then
			Me.BeginInvoke(New Action(Of String, Short)(AddressOf SetStepStatus), TestStep, Status)
		Else
			Dim blackCell As New DataGridViewCellStyle
			blackCell.ForeColor = Color.Black
			Dim blueCell As New DataGridViewCellStyle
			...
			If Status < 0 Or Status > 4 Then Status = 0
			For i = 0 To Me.LstTestStep.Rows.Count - 1
				If Me.LstTestStep.Rows(i).Cells(1).Value = TestStep Then Exit For
			Next
			If i > Me.LstTestStep.Rows.Count - 1 Then Exit Sub

			Select Case Status
				Case 0
					Me.LstTestStep.Rows(i).Cells(2).Value = ""
					Me.LstTestStep.Rows(i).Cells(2).Style = blackCell
				Case 1
					Me.LstTestStep.Rows(i).Cells(2).Value = "Running"
					Me.LstTestStep.Rows(i).Cells(2).Style = blueCell
				Case 2
					Me.LstTestStep.Rows(i).Cells(2).Value = "Pass"
					Me.LstTestStep.Rows(i).Cells(2).Style = greenCell
				Case 3
					Me.LstTestStep.Rows(i).Cells(2).Value = "Fail"
					Me.LstTestStep.Rows(i).Cells(2).Style = redCell
				Case 4
					Me.LstTestStep.Rows(i).Cells(2).Value = "Abort"
					Me.LstTestStep.Rows(i).Cells(2).Style = redCell
			End Select
		End If
	...
	Private Sub TxtBarcode_KeyPress(sender As Object, e As KeyPressEventArgs) Handles TxtBarcode.KeyPress
		If e.KeyChar = vbCr Then
			...
			If TestObj IsNot Nothing Then
				Me.DataGridView1.Rows.Clear()
				Dim status As Boolean = TestObj.CheckSN(TxtBarcode.Text, TxtPN.Text)
				If status Then
					ShowSelectedStep()
					TestObj.LoadTestData(TxtBarcode.Text, TxtPN.Text, LstTestStep.CurrentRow.Cells(1).Value.ToString)
				End If
	...
	Private Sub CmdCAL_Click(sender As Object, e As EventArgs) Handles CmdCAL.Click
		If TestObj IsNot Nothing Then
			Dim TestStep As String = LstTestStep.CurrentRow.Cells(1).Value.ToString
			TestObj.Calibrate(TestStep)
		End If
	End Sub
