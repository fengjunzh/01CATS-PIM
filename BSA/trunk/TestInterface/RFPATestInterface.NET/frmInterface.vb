...
Friend Class frmMain
    ...
    End Sub
    '=================Frank=======================
    Private Sub cqs_updatestatus(ByVal status As Integer)

        Try
            Dim cqs_chamber As New cqs_DAL.cqs_chamber(connstr)
            Dim cqs_computer As New cqs_DAL.cqs_computer(connstr)
            cqs_chamber.Update_CurChamberStatus(status, cqs_computer.Query_CurPC_BelongTo_ChamberID(cqs_computer.Query_CurPC_ComputerID(System.Environment.MachineName)))
        Catch
    ...
    Private Sub CmdRunAll_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles CmdRunAll.Click
        ...
        'check calibrate date===============================================
        ' Dim cal As New CATSPIM.CalibrationRecord
        ' INI_Read_Ca("")
...
ErrorHandler:
	...
	Public Sub SetStepStatus(ByVal TestStep As String, ByVal Status As Short)
		...
		Select Case Status
			...
			Case 4
				...
				Me.LstTestStep.Rows(i).Cells(1).Style = redCell
			Case 5
				Me.LstTestStep.Rows(i).Cells(1).Value = "Rework"
				Me.LstTestStep.Rows(i).Cells(1).Style = redCell
		End Select
	...
	Private Sub TxtBarcode_KeyPress(sender As Object, e As KeyPressEventArgs) Handles TxtBarcode.KeyPress
        ...
        Dim checksnsapEnable As Boolean = True

        If e.KeyChar = vbCr Then
            TxtPN.Text = TxtPN.Text.ToUpper()

            '============add generate SN for Cal funciton      20180613===========================
            If TxtPN.Text = "-110DBM_PIM_STD" Or TxtPN.Text = "LOW-PIM-LOAD" Then
                If TxtBarcode.Text.ToUpper.Contains("CAL0") And Trim(TxtBarcode.Text.ToUpper).Length < 8 Then
                    TxtBarcode.Text = TxtBarcode.Text.ToUpper + Format(Now(), "yyyyMMddHHmmss")
                Else
                    MsgBox("Please input SN again to start to calibration !.", MsgBoxStyle.Critical + MsgBoxStyle.OkOnly)
                    TxtBarcode.Text = ""
                End If
                checksnsapEnable = False
            End If

            If Trim(TxtBarcode.Text.ToUpper).Length > 30 Then ' 判断下SN的长度不能超过30，  20181123
            ...
            If TestObj IsNot Nothing Then
                ...
                '''''''------CheckSNSAP--------------------------------
                checksnsapEnable = False ' 暂时屏蔽掉 CheckSNSAP功能，因为SAPServer关机维护10/20
                If checksnsapEnable = True Then
                    If TSDP_Mode.Text.ToUpper = "PROD" Or TSDP_Mode.Text.ToUpper = "NPI" Then
                        Common.clsOperator.AddStatusMsg("Start to check SN with SAP......" & Now, True)
                        If TestObj.CheckSNSAP(TxtBarcode.Text, TxtPN.Text) = False Then
                            Me.DataGridView1.Rows.Clear()
                            ShowSelectedStep()
                            MsgBox("Remind : Check SN in SAP fail ,Please continue.", MsgBoxStyle.Critical + MsgBoxStyle.OkOnly)
                            'Return             
                        End If
                    End If
                End If
