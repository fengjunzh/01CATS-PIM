...
<System.Runtime.InteropServices.ProgId("clsGlobals_NET.clsGlobals")> Public Class clsGlobals
    ...
    ''add cal function===============================================
    ''Public Sub INI_Read_Cal(ByVal sel As Integer, ByVal phase As String)
    ...
    ''    Dim ept As New CATSPIM.Encryptor
    ''    If File.Exists(CalPathFile) Then '文件存在
    ''        '解密
    ''        Try
    ''            ' ept.DecryptFile(CalPathFile, tmpPath) '解密完成并把内容保存到tmp.tmp中,效果等于就是复制了一个文件。
    ''            If Read_INI_CalField(phase, phase & "_PC_name", “NA”, tmpPath) = "NA" Then Return


    ''            Read_Result = Read_INI_CalField(phase, phase & "_LowPim_Load", “NA”, tmpPath) ' 返回值
    ''            Read_Result = Read_INI_CalField(phase, phase & "_Standard_110dBm", “NA”, tmpPath) ' 返回值
    ...
    ''Public Function Read_INI_CalField(ByVal AppName As String, ByVal KeyName As String, ByVal Default_Renamed As String, ByVal filename As String) As String
    ''    Dim Valid, Size As Short
    ''    Dim ReturnString As String
    ''    ReturnString = Space(255)
    ''    Size = Len(ReturnString)
    ''    Valid = GetPrivateProfileString(AppName, KeyName, Default_Renamed, ReturnString, Size, filename)
    ''    Read_INI_CalField = Microsoft.VisualBasic.Left(ReturnString, Valid)
    ''End Function
