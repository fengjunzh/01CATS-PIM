Public Class Calculate
    ...
    Public Shared Function GetImRange(value As CATS.Model.cfg_imd_cfbox) As DataModels.AxisMaxMin
        ...
        End Try
    End Function
    Public Shared Function AverageM(x As List(Of DataModels.FrequencyPoint)) As Single ' add by tony 20190418 for STS-M,增加一个参数10， 20190729
        Dim temp As New List(Of Double)
        For Each t As DataModels.FrequencyPoint In x
            temp.Add(Math.Pow(10, （t.YData + 43） / 10))
        Next
        Return Math.Round(10 * Math.Log10(temp.Average()) - 43, 3)
    End Function
    'Public Shared Function AverageM(x As List(Of DataModels.FrequencyPoint)) As Single ' add by tony 20190418 for STS-M
    '    Dim temp As New List(Of Double)
    '    For Each t As DataModels.FrequencyPoint In x
    '        temp.Add(Math.Pow(10, t.YData + 43))
    '    Next
    '    Return Math.Round(Math.Log10(temp.Average()) - 43, 3)
    'End Function
	...
	Public Shared Function STS_A_Stdev(trace As Dictionary(Of String, List(Of DataModels.FrequencyPoint))) As Single
		Try
			...
			Dim resp As Single = 0
			Dim tmp As Single
			Dim fpFilter As DataModels.FrequencyPoint
			Dim fpFilterList As List(Of DataModels.FrequencyPoint)
			'Dim tmpValue As DataModels.FrequencyPoint
			Dim testNum As Short

			For testNum = 0 To trace(1).Count - 1

				fpFilterList = New List(Of DataModels.FrequencyPoint)

				For Each t In trace
					fpFilter = New DataModels.FrequencyPoint
					fpFilter.YData = t.Value(testNum).YData
					If fpFilter.YData < -160 Then fpFilter.YData = -160
					fpFilterList.Add(fpFilter)
				Next

				tmp = Stdev(fpFilterList)
				resp = IIf(tmp > resp, tmp, resp)
