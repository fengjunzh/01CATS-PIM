...
Public Class RosenbergerDevice
  ...
  End Sub
	...
	Public Sub SetRFPortPowerDBM(PORT As IIMDDevice.enumRFPORTS, PowerDBM As Double) Implements IIMDDevice.SetRFPower
		Try
			'Dim tmp As String, tmpDec As Decimal
			Select Case PORT
				Case IIMDDevice.enumRFPORTS.PORTA
					Send_And_Read("#SPWA" & String.Format("{0:D5}", CInt(PowerDBM * 10)))
					'tmp = Send_And_Read("#SPWA?")
					'tmpDec = CType(GetRegexField(tmp, "SPWA\d(\S+)"), Decimal) * 0.1
				Case IIMDDevice.enumRFPORTS.PORTB
					Send_And_Read("#SPWB" & String.Format("{0:D5}", CInt(PowerDBM * 10)))
					'tmp = Send_And_Read("#SPWB?")
					'tmpDec = CType(GetRegexField(tmp, "SPWB\d(\S+)"), Decimal) * 0.1
			End Select

			'If Math.Abs(tmpDec - PowerDBM) > 0.5 Then Throw New Exception("The Tx output power may be a problem, write=" & PowerDBM & ", read=" & tmpDec)

		Catch ex As Exception
			Throw New Exception("SetRFPortPowerDBM()::" & ex.Message)
		End Try
	...
	Public Sub SetRFPortPowerDBM(PowerDBM1 As Double, PowerDBM2 As Double) Implements IIMDDevice.SetRFPower
		...
		Try
			SetRFPortPowerDBM(IIMDDevice.enumRFPORTS.PORTA, PowerDBM1)
			SetRFPortPowerDBM(IIMDDevice.enumRFPORTS.PORTB, PowerDBM2)
		Catch ex As Exception
			Throw New Exception("SetRFPortPowerDBM()::" & ex.Message)
		End Try
  ...
  End Sub
    Public Function GetRFPower() As Double() Implements IIMDDevice.GetRFPower
        Try
            ...
            tmp = Send_And_Read("#SPWA?")
            resp(0) = CType(GetRegexField(tmp, "SPWA\d(\S+)"), Decimal) * 0.1

            tmp = Send_And_Read("#SPWB?")
            resp(1) = CType(GetRegexField(tmp, "SPWB\d(\S+)"), Decimal) * 0.1
        ...
        End Try
    End Function
    Public Sub SetPIMFreqMHz(ByVal FreqMHz1 As Double) Implements IIMDDevice.SetPIMFreqMHz
