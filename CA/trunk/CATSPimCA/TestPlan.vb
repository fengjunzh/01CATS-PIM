...
Public Class TestPlan
    ...
    Private tPhase As New TestSpec.TestPhase
    ...
    End Sub
    Public Sub PreTest(TestPhase As String) Implements ITestPlan.PreTest
        pAbortFlag = False
    ...
    Public Sub RunTest(Barcode As String, TestPhase As String, ByRef Cancel As Boolean) Implements ITestPlan.RunTest
        Try
            ...
            pRTP.AlgoParas = GetAlgoParasBySpecMainId(tPhase.SpecMainId)
            ...
            If TestModules.RunTestPhase(tPhase) = False Then
...
#Region "OPC UA Operation"
    ...
    Public Async Sub ReadPlcAsync() Implements ITestPlan.ReadPlcAsync
        Try
            ...
            Dim pimTrig As String = If(gPimTestConfig.Pretest, OpcUaConst.PIM1FixtureReadyInt, OpcUaConst.PIM2FixtureReadyInt)
    ...
    Public Function InitInstrument() As Boolean Implements ITestPlan.InitInstrument
        Try
            If InitPimInstrument(tPhase) = False Then
                GUI.RecordResult("Init Instr Fail", -1, 0, 0)
                AddStatusMsg("Initialize test instruments failed, please check connection and configuration!")
                Return False
            End If
            GUI.WritePlcValues(numTestPhases.PreparedReady)
            Return True
        Catch ex As Exception
            Throw New Exception("InitInstrument()::" & vbCrLf & ex.Message)
        End Try
    End Function

    Public Sub ClosePimInstrument() Implements ITestPlan.ClosePimDevice
        Try
            ClosePimDevice()
        Catch ex As Exception
            Throw New Exception("ClosePimDevice()::" & vbCrLf & ex.Message)
        End Try
    End Sub
#End Region
End Class
