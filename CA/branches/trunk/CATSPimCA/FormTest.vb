...
Public Class FormTest
    ...
    Private m_2ToneReq As Boolean
    Private Const Adjacent2PVarLimit As Integer = 10
    Private m_HighPowerTest As Boolean
    Public Sub New()
    ...
    Private Sub AddPointToList(ByRef pointList As List(Of DataModels.FrequencyPoint),
        ...
        Try
            ...
            Select Case descr
                ...
                Case "STS-B"
                    If m_testTrace.StsB.ContainsKey(seriesId) = False Then
                        m_testTrace.StsB.Add(seriesId, New List(Of DataModels.FrequencyPoint))
                    End If
                    m_testTrace.StsB(seriesId).Add(imdPoint)
            End Select
    ...
    Private Sub TwoToneFrequencyTest(cycle As Short, seriesId As Short, ffReq As CATS.Model.cfg_imd_ffbox)
        Try
            ...
            Dim randomNumber As Integer = rand.Next(1, 5)
...
#If INSTR_NORMAL_TEST Then
            pPimDev.CheckPowerVersion(CSng(ffReq.c1_power))
            ...
            If m_HighPowerTest And ffReq.c1_power > 43 Then
                Dim status As String
                If ffReq.c1_freq > ffReq.c2_freq Then
                    status = "#MDNL0"
                Else
                    status = "#MDNL1"
                End If
...
#End If
                ...
                If imdPoints.Count = randomNumber And gPimTestConfig.SnReader = True Then
                    Dim readSN As String = ReadScanner.Trim
                    If readSN <> pRTP.barcode Then Throw New Exception(String.Format("Reading SN <{0}> Mismatch with DUT SN <{1}>, Stop Test!", readSN, pRTP.barcode))
                End If
...
#If INSTR_NORMAL_TEST Then
            ...
            If gSelectedInstrument.Vendor.Trim.ToUpper = "ZULU".ToUpper And pRTP.phase = "PIM3500" Then
                pPimDev.RFPowerRampUp(txrFreqs(0), txlFreqs(0), "DOWNSWEEP")
            Else
...
#If INSTR_NORMAL_TEST Then
                    ...
                    pPimDev.CorrectRFPower_TwoPort()
                    If gSelectedInstrument.Vendor.Trim.ToUpper = "ZULU".ToUpper Then
                        If j = 1 Then
                            MyDelay(400)
                        Else
                            MyDelay(150)
...
#If INSTR_NORMAL_TEST Then
                    pPimDev.SetPIMFreqMHz(trxf.RxFreq)
                    rxValue = ReadImdValue()
#Else
                    rxValue = GetVirtualImValue()
#End If
                    ...
                    If cfReq.freq_desc.Contains("STS-A") Then
                        AddPointToList(imdPoints, j, trxf.TxlFreq, trxf.TxrFreq, trxf.RxFreq, dspPoint.X, dspPoint.Y, "STS-A")
                    ElseIf cfReq.freq_desc.Contains("STS-B") Then
                        AddPointToList(imdPoints, j, trxf.TxlFreq, trxf.TxrFreq, trxf.RxFreq, dspPoint.X, dspPoint.Y, "STS-B")
                    ElseIf cfReq.freq_desc.Contains("STS-C") Then
                        AddPointToList(imdPoints, j, trxf.TxlFreq, trxf.TxrFreq, trxf.RxFreq, dspPoint.X, dspPoint.Y, "STS-C")
                    Else
                        MsgBox("Not found " & cfReq.freq_desc & " config, please contact test engineer !", vbExclamation + MsgBoxStyle.OkOnly)
                    End If
                    ...
                    Else
                        m_txtReport.WriteTestPoint("STS-B" & j, imdPoints(imdPoints.Count - 1))
                    End If
...
#End If
    ...
    Private Function DisplaySweepPointIn2ToneChart() As DataModels.FrequencyPoint
        Try
            ...
            resp = Calculate.FindWorstPoint(m_testTrace.Sweep)
        ...
        End Try
    End Function
    Private Function DisplaySweepPointInSweepFreqChart(fpoint As FrequencyPoint) As DataModels.FrequencyPoint
        Try

            pMTDisplay.SetMarker(fpoint, Color.Red)
    ...
    Private Sub RunAllTest()
        Try
            m_HighPowerTest = False
            If HiPowerCheck() Then
                m_HighPowerTest = True
            End If
            gCFTestTimes = 0
            gFFTestTimes = 0
            gGetPassVirtualVale = True
            Dim TwoToneExe As Boolean = False

            Dim TestCount As Int16 = 1
            If pRTP.family = "Input Cable" Then
                TestCount = 2
            End If
            If pRTP.M_product_main.product_name.StartsWith("ML") Or pRTP.M_product_main.family = "MLOCK" Then
                TestCount = 3
            End If
            GUI.RetryCountMax = TestCount - 1
            GUI.RetryCountRest = TestCount - 1
            ...
            Do While (FExitDo = False)
                ...
                Try
                    ...
                    If m_testItem.spec_imd_detail.vibe_enable And gPimTestConfig.Vibration.Enable Then
                        'close clamp
                        If GUI.Dynamic = True And pPlantCode = "CN10" Then
                            If pVibCtrl.FixtureReady2Go = False Then
                                'stop fixture
                                pVibCtrl.StopFixture()
                                'open clamp
                                pVibCtrl.OpenClamp()
                                Throw New Exception("Vibration deivce is not working, Stop Test!")
                            End If
                        End If
                        pVibCtrl.CloseClamp()
                        MyDelay(1000)
                        ...
                        'start fixture 
                        pVibCtrl.StartFixture()
                    End If

                    Dim pr As New TestReport.ReportModule

                    If pRTP.family = "Input Cable" Then
                        If Not m_2ToneReq Then PreRun()
                    Else
                        PreRun()
                    End If
                    ...
                    ' Start rotate fixture and run test
                    ' TODO : need to optimize rotate fixture control logic
                    GUI.WritePlcValues(numTestPhases.RotateFixture)

                    If pRTP.family = "Input Cable" Then
                        If PSFReq IsNot Nothing Then        'sweep test
                            MTSweepFrequencyTest()
                            'display sweep point
                            fpoint = DisplaySweepPointIn2ToneChart()
                        ElseIf PFFReq IsNot Nothing And m_2ToneReq Then    '2 tone test
                            If fpoint.TxlFreq > 0 Then PFFReq.c1_freq = fpoint.TxlFreq : PFFReq.c2_freq = fpoint.TxrFreq : PFFReq.imd_freq = fpoint.RxFreq
                            gFFTestTimes += 1
                            ChartReLayoutStsT()
                            ...
                            m_2ToneReq = False
                            TwoToneExe = True
                        ElseIf PCFReq IsNot Nothing And m_2ToneReq = False Then    'custom sweep test
                            gCFTestTimes += 1
                            ChartLayout()
                    ...
                    Else
                        If PSFReq IsNot Nothing Then        'sweep test
                            ...
                            'display sweep point
                            fpoint = DisplaySweepPointIn2ToneChart()
                            If fpoint IsNot Nothing Then
                                DisplaySweepPointInSweepFreqChart(fpoint)
                            End If
                        End If
                        If PFFReq IsNot Nothing Then    '2 tone test
                            If fpoint.TxlFreq > 0 Then PFFReq.c1_freq = fpoint.TxlFreq : PFFReq.c2_freq = fpoint.TxrFreq : PFFReq.imd_freq = fpoint.RxFreq
                            MTTwoToneFrequencyTest()
                            m_testTrace.Lambda = Calculate.LambdaTrace(m_testTrace.TwoToneFilter)
                            pMTDisplay.DisplayLamdaPoint(2, m_testTrace.TwoToneFilter, fpoint)
                            pMTDisplay.RefreshLambdaChart(1, m_testTrace.Lambda)
                        End If
                        If PCFReq IsNot Nothing Then    'custom sweep test
                            MTCustomFrequencyTest()
                        End If
                    End If

                    If GUI.Dynamic = True And pPlantCode = "CN10" And gPimTestConfig.Vibration.Enable Then
                        If pVibCtrl.FixtureRunComplete = False Then
                            'stop fixture
                            pVibCtrl.StopFixture()
                            'open clamp
                            pVibCtrl.OpenClamp()
                    ...
                    If crList.Count = 0 Then Throw New Exception("no test criteria created")

                    Dim test_pass As Boolean = True
                    For Each c In crList
                        test_pass = test_pass And IIf(c.Value.meas_status = "P", True, False)
                    Next

                    ' Check if 2 tone test is required
                    CheckTwoToneRequired(test_pass)
                ...
                Finally
                    Try
                        ...
                        For Each c In crList
                            test_pass = test_pass And IIf(c.Value.meas_status = "P", True, False)
                        Next
                        GUI.RetryCountRest = TestCount - TestRec
                        If test_pass Then
                            If pRTP.family = "Input Cable" Then
                                If gCFTestTimes = 1 And gFFTestTimes = 0 Then 'first time custom freq test pass            
                                    If gAdjacent2PVar > Adjacent2PVarLimit Then 'check if adjacent 2 points variance is bigger than 15dB
                                        Load2ToneCfg()
                                        'm_2ToneReq = True
                                        FExitDo = False
                                        retry = False
                                    Else 'first time custom freq test pass and adjacent 2 points variance is less than 15dB, exit test
                                        FExitDo = True : Exit Try
                                    End If
                                ElseIf gFFTestTimes = 1 Then 'first ff test pass then exit test
                                    FExitDo = True : Exit Try
                                ElseIf gCFTestTimes = 2 Then 'retry test pass then need 2 tone test
                                    Load2ToneCfg()
                                    'm_2ToneReq = True
                                    FExitDo = False
                                    retry = False
                                End If
                            Else ' other than input cable first test pass then exit test
                                FExitDo = True : Exit Try
                            End If
                        Else 'test fail
                            gGetPassVirtualVale = True
        ...
        End Try
    End Sub
    Private Sub CheckTwoToneRequired(test_pass As Boolean)
        Try
            If pRTP.family = "Input Cable" Then
                m_2ToneReq = False
                If test_pass = True Then
                    If gCFTestTimes = 1 And gFFTestTimes = 0 Then 'first time custom freq test pass            
                        If gAdjacent2PVar > Adjacent2PVarLimit Then 'check if adjacent 2 points variance is bigger than 15dB
                            If gFFTestTimes = 0 Then 'check if first 2 tone test done
                                m_2ToneReq = True
                            End If
                        End If
                    ElseIf gCFTestTimes = 2 Then 'retry test pass then need 2 tone test
            ...
            End If
        Catch ex As Exception
            Throw New Exception("FormTest.TwoToneRequired()::" & ex.Message)
        End Try
    End Sub
    Private Sub Load2ToneCfg()
        Try
            If pRTP.family = "Input Cable" And m_testItem.cfg_imd_main.descr.Contains("STS-C") And Not m_testItem.cfg_imd_main.descr.Contains("STS-T") Then
                Dim cimML As List(Of CATS.Model.cfg_imd_main) = (New CATS.BLL.cfg_imd_mainManager).SelectByCfgImdFreqbandId(m_testItem.cfg_imd_main.cfg_imd_freq_band_id)
                TwoTone_CfgM = cimML.Find(Function(o) o.descr.Contains("STS-T") And o.descr.Contains("t10"))
                If TwoTone_CfgM IsNot Nothing Then PFFReq = TestModules.GetFixedFreqs(TwoTone_CfgM, pRTP.instr_vendor_id)
            End If
        Catch ex As Exception
            Throw New Exception("FormTest.Load2ToneCfg()::" & ex.Message)
        End Try
    End Sub
    ...
    Private Sub btnExit_Click(sender As System.Object, e As System.EventArgs) Handles btnExit.Click
        ...
        End Try
    End Sub
    Private Sub ChartLayoutStsABC(fdescr As String)
        Try
            ...
            If fdescr.Contains("STS-A") Then
                g5.Text = "STS-A"
                gbSTSC.Text = "[ STS-A ]"
            ElseIf fdescr.Contains("STS-C") Then
                g5.Text = "STS-C"
                gbSTSC.Text = "[ STS-C ]"
            Else
                g5.Text = "STS-B"
            ...
            gbSTSC.Left = 10
            gbSTSC.Top = 50
            gbSTSC.Width = (Me.Width - 30) * 5 / 6
            gbSTSC.Height = Me.Height - 100

            gbResult.Left = gbSTSC.Left + gbSTSC.Width + 10
            gbResult.Top = gbSTSC.Top
            gbResult.Width = (Me.Width - 30) / 6
            gbResult.Height = gbSTSC.Height
    ...
    Private Sub ChartLayout()
        ...
        Try
            ...
            ElseIf fdescr.Contains("STS-J") Or fdescr.Contains("STS-K") Then
                ChartLayoutStsJK()
            ElseIf fdescr.Contains("STS-A") Or fdescr.Contains("STS-B") Or fdescr.Contains("STS-C") Then
                ChartLayoutStsABC(fdescr)
    ...
    Public Sub ChartReLayoutStsT()

        Dim fdescr As String = ""

        Try
            If TwoTone_CfgM Is Nothing Then Return

            fdescr = TwoTone_CfgM.descr.ToUpper.Trim
