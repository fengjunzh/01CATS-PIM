Public Class Calculate
	...
	Public Shared Function STS_A_Stdev(trace As Dictionary(Of String, List(Of DataModels.FrequencyPoint))) As Single
		...
		End Try
	End Function
	Public Shared Function STS_C_Adjacent_2Points_Var(trace As Dictionary(Of String, List(Of DataModels.FrequencyPoint))) As Single
		Try
			Dim resp As Single = 0
			Dim tmp As Single
			Dim pointNum As Integer = trace(1).Count
			For i As Integer = 1 To trace.Count
				For j As Integer = 0 To pointNum - 2
					tmp = Math.Abs(trace.Item(i)(j + 1).YData - trace.Item(i)(j).YData)
...
#Region "Find point"
	...
	End Function
	Public Shared Function FindWorstPoint(pointList As List(Of DataModels.FrequencyPoint)) As DataModels.FrequencyPoint
		Try
			Dim res As New DataModels.FrequencyPoint
			res.YData = -999
			For Each p As DataModels.FrequencyPoint In pointList
				If p.YData > res.YData Then
					res = p
				End If
			Next
		...
		Catch ex As Exception
			Throw New Exception("FindWorstPoint()::" & ex.Message)
		End Try
    ...
    Public Shared Function GetGRRSamplesRange(ByVal Runs As List(Of GC.SamplePoint)) As Single
        Try
            Dim resp As Single

            Dim min As Single = 1000000
            Dim max As Single = -1000000

            For Each point As GC.SamplePoint In Runs
                If point.Peak > max Then max = point.Peak
                If point.Peak < min Then min = point.Peak
            Next
        ...
        Catch ex As Exception
            Throw New Exception("TestClass.Calculate.GetArrayRange()::" & ex.Message)
        End Try
    End Function
#End Region
End Class
