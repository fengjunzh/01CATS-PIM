			...
			Try
				...
				If trace.StsC.Count > 0 Then
					...
					With tp_model
						...
						End If
						.meas_status = IIf((.meas_value >= .meas_ll And .meas_value <= .meas_ul And .meas_value > gDiscard_value), "P", "F")
					End With
				...
				If trace.StsC.Count > 0 Then
					tp_crItem = "STS-C_STDEV"
					tp_cq_model = tcrList(tp_crItem)
					tp_prItem = item.spec_detail.meas_item & "_" & tp_crItem
					tp_value = Calculate.STS_A_Stdev(trace.StsC)
					tp_model = New CATS.Model.meas_criteria
					...
					With tp_model
						...
						Else
							...
							If pRTP.family = "Input Cable" Then
								If gCFTestTimes = 2 And gFFTestTimes = 1 Then
									.meas_ul = 0.1
								Else
									.meas_ul = 1
								End If
						...
						End If
						.meas_status = IIf((.meas_value >= .meas_ll And .meas_value <= .meas_ul), "P", "F")
						tp_stastb = tp_stastb And IIf(.meas_status = "P", True, False)
					End With
					resp.Add(tp_prItem, tp_model)
				End If
				'#End Region
				...
				If trace.StsB.Count > 0 Then
					tp_crItem = "STS-B_MAX"
					tp_cq_model = tcrList(tp_crItem)
					tp_prItem = item.spec_detail.meas_item & "_" & tp_crItem
					tp_value = Calculate.STS_A_Max(trace.StsB)
					tp_model = New CATS.Model.meas_criteria
				...
				If trace.StsB.Count > 0 Then
					tp_crItem = "STS-B_STDEV"
					tp_cq_model = tcrList(tp_crItem)
					tp_prItem = item.spec_detail.meas_item & "_" & tp_crItem
					tp_value = Calculate.STS_A_Stdev(trace.StsB)
					tp_model = New CATS.Model.meas_criteria
				...
				If trace.StsC.Count > 0 Then
					gAdjacent2PVar = Calculate.STS_C_Adjacent_2Points_Var(trace.StsC)
				End If
		...
		Public Sub PrintTResultDcf(meas_criteriaList As Dictionary(Of String, CATS.Model.meas_criteria))
			Try
				...
				For Each p As KeyValuePair(Of String, CATS.Model.meas_criteria) In meas_criteriaList
					GUI.RecordResult(p.Value.meas_item, p.Value.meas_value, p.Value.meas_ll, p.Value.meas_ul)
				Next
		...
		Public Sub PrintTResultGui(form As FormTest, meas_criteriaList As Dictionary(Of String, CATS.Model.meas_criteria))
			Try
				...
				For Each p As KeyValuePair(Of String, CATS.Model.meas_criteria) In meas_criteriaList
					...
					ElseIf p.Key.ToString.Contains("STS-B_MAX") Then
						form.txt_StsA_max.Text = Math.Round(p.Value.meas_value, 1)
						form.txt_StsA_max.ForeColor = IIf(p.Value.meas_status = "P", Color.Black, Color.Red)
					ElseIf p.Key.ToString.Contains("STS-B_STDEV") Then
						form.txt_StsA_stdev.Text = Math.Round(p.Value.meas_value, 2)
						form.txt_StsA_stdev.ForeColor = IIf(p.Value.meas_status = "P", Color.Black, Color.Red)
					ElseIf p.Key.ToString.Contains("STS-C_MAX") Then
						form.txt_StsA_max.Text = Math.Round(p.Value.meas_value, 1)
						form.txt_StsA_max.ForeColor = IIf(p.Value.meas_status = "P", Color.Black, Color.Red)
					ElseIf p.Key.ToString.Contains("STS-C_STDEV") Then
						form.txt_StsA_stdev.Text = Math.Round(p.Value.meas_value, 2)
						form.txt_StsA_stdev.ForeColor = IIf(p.Value.meas_status = "P", Color.Black, Color.Red)
					End If
				Next
	...
	Public Class TxtReport
		...
		Public Sub WriteSweepFreqHead()
			Try
				...
				sw.WriteLine("Time:" & CType(GUI.StartTestTime, DateTime).ToString("yyyy-MM-dd hh:mm:ss"))
...
#Region "PicReport"
	Public Class PicReport
		...
		Public Sub New(form As Form, picPath As String)
			m_form = form
			m_picPath = picPath
		End Sub
...
#Region "XmlReport"
	Public Class XmlReport
		...
		Public Shared Property PlotPath As String
			...
			End Set
		End Property
		Public Shared Function WriteHead(serialNumber As String) As TestReport.XmlFramework.THead
			Try
		...
		Private Shared Function AddTestExtend(trace As DataModels.TestTrace) As TestReport.XmlFramework.TExtend
			Try
				...
				If gLastMeasStopTime IsNot Nothing Then resp.MeasValue.Add(gLastMeasStopTime)
		...
		Public Shared Function WriteTestItem(spec As CATS.Model.cq_spec_imd_details,
			...
			Try
				...
				If trace.StsB.Count > 0 Then
					For Each trc In trace.StsB
						tpTrcs.Add(AddTestTrace(traceIdx, "STS-B" & traceIdx, trc.Value))
						traceIdx += 1
					Next
				End If
				...
				If trace.StsC.Count > 0 Then
					For Each trc In trace.StsC
						tpTrcs.Add(AddTestTrace(traceIdx, "STS-C" & traceIdx, trc.Value))
						traceIdx += 1
					Next
				End If
