...
Public Class FormGRR
    ...
    Private _InstrPara As CATSPimConfig.LocalConfig.InstrPara
...
#Region "FormGRR Property"
    ...
    Public Property InstrPara As LocalConfig.InstrPara
        Get
            Return _InstrPara
        End Get
        Set(value As LocalConfig.InstrPara)
            _InstrPara = value
        End Set
    End Property
...
#End Region
    Private Sub linkGCProcedure_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles linkGCProcedure.LinkClicked
        ...
        ' Navigate to a URL.
        System.Diagnostics.Process.Start("http://home.commscope.com/Quality/Andrew/qsys/Business%20Groups/Cable%20Products/Cable%20Assys/Assembly%20Test%20Procedures/82CAW04-06%20-%20PIM%20Gage%20Capability%20Procedure.doc")
    End Sub
    ...
    Private Sub FormGRR_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            ' Connect CATS RD server - asz-42jc23x
            'Dim CatsConn As New CATS.BLL.CATSManager
            'CatsConn.ActivateCATS(My.MySettings.Default.connString)

            ' Get instrument set up
            InstrPara = pAppCfg.GetInstrumentConfig()

            If InstrPara Is Nothing Then
                MsgBox("Don't find intrument configuration, please setup device", MsgBoxStyle.Critical + MsgBoxStyle.OkOnly)
                FormConfig.ShowDialog()
                Return
            End If

            AnalyzerVendor = InstrPara.Vendor
            AnalyzerModel = InstrPara.Model
            FreqBand = InstrPara.BandName
            ComPort = InstrPara.Address
            BandIdx = InstrPara.BandIdx
...
#End If
    ...
    Private Sub LoadProcedure()
        Try
            ...
            If GC_Procedure_List IsNot Nothing Then
                GC_Procedure_Model = GC_Procedure_List(0)
                ...
                Dim expiry As DateTime = GC_Procedure_Model.TimeCompleted.AddDays(7)
                If temp > expiry Then
                    IsExpired = True
    ...
    Private Function InitVirtualPimDevice() As DataModels.Instrument
        Try
            ...
            dev.Idn = "Rosenberger HF-Technik GmbH & Co. KG,IM-19AWS    Serial No. 031IM-E2004,Firmware v7.53 [2015-05-11]"
